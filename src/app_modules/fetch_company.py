...
fragment_company = QueryFragment(
    name="company",
    on_type="Company",
...
def fetchSubjectCompany(id: int) -> SubjectCompany:
    ...
    response: Response = requests.post(url=craft_url,headers={"X-Craft-Api-Key": craft_key},json=CraftPayload(query=query, variables=variables))
